...
export const initDB = async () => {
  try {
    console.log('Connecting to MySQL/TiDB and initializing schema...');

    // ---------------------------------------------------------
    // IMPORTANT:
    // The database "people_voice" should already exist in TiDB.
    // We connect directly to it using the secure TLS connection.
    // ---------------------------------------------------------
    ...
    try {
      const [cols] = await dbPool.query(`
        ...
        WHERE TABLE_SCHEMA = ?
          ...
          AND COLUMN_NAME IN ('confidence', 'original_command')
      `, [DB_NAME]);

      const existingColNames = cols.map(
      ...
      if (!existingColNames.includes('confidence')) {
        ...
        console.log(
          'Migration: Added confidence column'
        );
      }
      ...
      if (!existingColNames.includes('original_command')) {
        await dbPool.query(`
          ALTER TABLE inventory_transactions
          ADD COLUMN original_command VARCHAR(255) NULL
        `);

        console.log(
          'Migration: Added original_command column'
        );
      }
    ...
    if (Number(rows[0].count) === 0) {
      ...
      console.log(
        'Seeding initial demo products and transactions...'
      );
    ...
    console.log(
      `Database connected and initialized successfully (${DB_NAME})`
    );
